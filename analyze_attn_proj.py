...
def process_attention_data_with_rank(proj_results_path, result_path, thresholds, HEADS, subtask_paren, paren_token_ids):
    """
...
def accuracy_generalization(result_path, paren_token_ids, model, threshold_value=0.7, threshold_index=0):
    """
...
def process_proj_results_with_rank(model, folder_name):
    HEADS = [(layer, head) for layer in range(model.cfg.n_layers) for head in range(model.cfg.n_heads)]
    proj_results_path = f"results/projections/{folder_name}/attn/proj"
    thresholds = [0.5, 0.6, 0.7, 0.8, 0.9] # threholds for calculating accuracy, precision, recall, f1
    result_path = f"results/attn_results/{folder_name}/with_rank"
    create_results_dir(result_path)
    paren_token_ids = get_paren_logit_idx(folder_name)
    for paren in tqdm(paren_token_ids):
        subtask_paren = model.tokenizer.decode(paren)
        process_attention_data_with_rank(proj_results_path, result_path, thresholds, HEADS, subtask_paren=subtask_paren, paren_token_ids=paren_token_ids)       
    ...
    accuracy_generalization(result_path, paren_token_ids, model, threshold_value=0.9) #prev: 0.7
...
def process_proj_results(model, folder_name):
    ...
    proj_results_path = f"results/projections/{folder_name}/attn/proj"
    thresholds = [0.5, 0.6, 0.7, 0.8, 0.9] # threholds for calculating accuracy, precision, recall, f1
    result_path = f"results/attn_results/{folder_name}"
    create_results_dir(result_path)
    paren_token_ids = get_paren_logit_idx(folder_name)
    for paren in tqdm(paren_token_ids):
        subtask_paren = model.tokenizer.decode(paren)
...
def main():
    ...
    for m in models:
        ...
        process_proj_results_with_rank(model, folder_name)
    ...
    results_path = "results/meta_results/generalization_heads.json"
